import {
  ...
  import { HEAR_ABOUT_US, LOOKING_FOR } from "../../constants";
  ...
  function LookingFor() {
    ...
    const getReasons = async () => {
      try {
        ...
        setLookingFor([
          {
            _id: "1",
            reason: "Looking for a friend",
          },
          {
            _id: "2",
            reason: "Looking for a partner",
          },
        ]);
      } catch (error) {
        console.log(error);
    ...
    const handleDelete = async (id) => {
      try {
        const response = await deleteData(`${LOOKING_FOR}/${id}`);
      ...
      <>
        ...
        <div className="grid grid-cols-12 gap-6 mt-5">
          <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2 justify-between">
            <div>
              {/* <p className="text-lg font-medium">Cancellation Reason</p> */}
            </div>
            <div>
              <Link to={"/add-looking-for"}>
                ...
                <tbody>
                  {lookingFor?.map((link, index) => (
                    <tr key={index} className="intro-x">
                      ...
                      <td className="text-center">{link.reason || "-"}</td>
  
                      <td className=" w-56">
                        <div className="flex justify-center items-center ">
                          <div
                            onClick={() =>
                              navigate("/add-looking-for", {
                                state: { data: link },
                              })
                            }
