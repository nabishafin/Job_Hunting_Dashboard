...
const AddCancellationReason = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      let url = isEdit ? `${CANCELLATION_REASON}/${linksData._id}` : CANCELLATION_REASON;
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          toast.success(response?.data?.message || "Cancellation reason saved successfully");
          setLoading(false);
    ...
    <div className="p-4 flex-1 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        <p className="pb-6 text-lg font-semibold">
          {isEdit ? "Edit" : "Add "} cancellation reason
        </p>
      </div>
