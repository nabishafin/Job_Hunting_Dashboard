...
import { CATEGORY_DROPDOWN } from "../../constants";
...
const AddCategoryDropdown = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      let url = isEdit
        ? `${CATEGORY_DROPDOWN}/${linksData._id}`
        : CATEGORY_DROPDOWN;
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          toast.success(
            response?.data?.message || "Category dropdown saved successfully"
          );
          setLoading(false);
          navigate("/category-option");
        } else {
          toast.error("An error occurred while saving the category dropdown");
        }
      } catch (error) {
        toast.error(
    ...
    <div className="p-4 flex-1 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        <p className="pb-6 text-lg font-semibold">
          {isEdit ? "Edit" : "Add "} Category Dropdown
        </p>
      </div>
