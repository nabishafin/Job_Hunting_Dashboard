...
const AddMealStyleDropdown = () => {
  ...
  const formik = useFormik({
    initialValues: {
      mealStyle: linksData?.mealStyle || "",
    },
    validationSchema: Yup.object({
      mealStyle: Yup.string().required("Meal style option is required"),
    }),
    onSubmit: async (values) => {
      setLoading(true);

      let url = isEdit
        ? `${MEAL_STYLE_DROPDOWN}/${linksData._id}`
        : MEAL_STYLE_DROPDOWN;
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          toast.success(
            response?.data?.message || "Meal style dropdown saved successfully"
          );
          setLoading(false);
          navigate("/meal-style-option");
        } else {
          toast.error("An error occurred while saving the meal style dropdown");
        }
      } catch (error) {
        toast.error(
    ...
    <div className="p-4 flex-1 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        <p className="pb-6 text-lg font-semibold">
          {isEdit ? "Edit" : "Add "} Meal Style Dropdown
        </p>
      </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="w-full">
            ...
            <input
              type="text"
              name="mealStyle"
              value={values.mealStyle}
              placeholder="Please enter the meal style"
              className="w-full p-2 mt-2 border rounded-md"
              onChange={handleChange}
              onBlur={handleBlur}
            />
            {touched.mealStyle && errors.name && (
              <div className="text-red-500 text-sm">{errors.mealStyle}</div>
            )}
          </div>
...
export default AddMealStyleDropdown;
