...
const BASE_URL = import.meta.env.VITE_BASE_URL || "http://localhost:5001/api/v1";

// Create a base query with automatic token refresh
const baseQuery = fetchBaseQuery({
  baseUrl: BASE_URL,
  prepareHeaders: (headers, { getState }) => {
    const token = getState().user.accessToken || localStorage.getItem("token");
...
export const baseApi = createApi({
  ...
  tagTypes: ["auth", "dashboardStats", "admin-users", "earnings-overview", "Faqs", "payments", "JobStatus", "TimeSlots"],
  endpoints: () => ({}),
