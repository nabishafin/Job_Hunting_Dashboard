...
export function parseDateRange(dateRange) {
  ...
  const getLast30Days = () => {
    ...
    return `${last30Days.toISOString()} - ${today.toISOString()}`; // ISO format for consistent parsing
  };

  try {
    // If no date range is provided, calculate the last 30 days
    const rangeToParse = dateRange || getLast30Days();
    const [startDateString, endDateString] = rangeToParse.split(" - ");
    const startDate = formatDate(new Date(startDateString.trim()));
    const endDate = formatDate(new Date(endDateString.trim()));

    return { startDate, endDate };
  } catch (error) {
    console.error("Invalid date range format", error);
    throw new Error("Invalid date range format. Expected format: '1 Dec, 2024 - 31 Jan, 2025'.");
  }
}
...
export function formatFraction(input) {
  ...
  if (match) {
      ...
      return unicodeFractionMap[input] || input;
  }
  return input;
}
